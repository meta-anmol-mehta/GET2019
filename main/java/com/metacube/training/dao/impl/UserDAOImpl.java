...
import com.metacube.training.mapper.UserCredentialMapper;
...
private final String SQL_FIND_CREDENTIAL = "select * from usercredential where username = ? and password= ? ";
private final String SQL_INSERT_USER_CREDENTIAL = "insert into usercredential(username,password) values(?,?)";
	...
	public boolean getUsersCredentials(String username, String password) {
		...
		try{
		List<UserCredential> credetials=jdbcTemplate.query(SQL_FIND_CREDENTIAL, new Object[] { username,password }, new UserCredentialMapper());
		if(credetials.isEmpty()) {
			 jdbcTemplate.update(SQL_INSERT_USER_CREDENTIAL, username,password);
			return true;
		}
		else{
			return false;
